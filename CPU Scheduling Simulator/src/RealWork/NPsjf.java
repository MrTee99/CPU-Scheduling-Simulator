...
public class NPsjf {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NPsjf window = new NPsjf();
					window.frmNPsjf.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() {
		frmNPsjf = new JFrame();
		frmNPsjf.setBounds(100, 100, 771, 626);
		frmNPsjf.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmNPsjf.setVisible(true);
		frmNPsjf.setResizable(false);
		frmNPsjf.getContentPane().setBackground(SystemColor.windowBorder);
		frmNPsjf.getContentPane().setLayout(null);
		frmNPsjf.setLocationRelativeTo(null);
		frmNPsjf.setTitle("CPU Scheduling Simulator");
		
		JPanel panel = new JPanel();
		panel.setBackground(SystemColor.windowBorder);
		panel.setBorder(new MatteBorder(10, 5, 10, 5, (Color) new Color(128, 128, 128)));
		panel.setBounds(0, 0, 765, 598);
		frmNPsjf.getContentPane().add(panel);
		panel.setLayout(null);
		
		JLabel lblCpuSchedulingSimulator = new JLabel("Non-Preemptive SJF Scheduling");
