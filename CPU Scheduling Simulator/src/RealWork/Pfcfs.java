...
public class Pfcfs {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Pfcfs window = new Pfcfs();
					window.frmPfcfs.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() {
		frmPfcfs = new JFrame();
		frmPfcfs.setBounds(100, 100, 771, 626);
		frmPfcfs.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmPfcfs.setVisible(true);
		frmPfcfs.setResizable(false);
		frmPfcfs.getContentPane().setBackground(SystemColor.windowBorder);
		frmPfcfs.getContentPane().setLayout(null);
		frmPfcfs.setLocationRelativeTo(null);
		frmPfcfs.setTitle("CPU Scheduling Simulator");
		
		JPanel panel = new JPanel();
		panel.setBackground(SystemColor.windowBorder);
		panel.setBorder(new MatteBorder(10, 5, 10, 5, (Color) new Color(128, 128, 128)));
		panel.setBounds(0, 0, 765, 598);
		frmPfcfs.getContentPane().add(panel);
		panel.setLayout(null);
		
		JLabel lblCpuSchedulingSimulator = new JLabel("Preemptive FCFS Scheduling");
		 ...
		 * 
		 */
		JPanel[] GC = new JPanel[16];
		int Xaxis = 170;
		for(int i =0; i<16; i++) {
			GC[i] = new JPanel();
			GC[i].setBackground(new Color(127, 255, 0));
			GC[i].setVisible(false);
			GC[i].setBounds(Xaxis, 437, 30, 67);
			panel.add(GC[i]);
			Xaxis = Xaxis+36;
		}
		
		JLabel[] GCPid = new JLabel[16];
		for(int i=0; i<16; i++) {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				for(int i=0; i<=16; i++) {
					...
					if(AT[j] > TimePassed) {
						...
						GCPid[i].setText("Idle");	
						while(TimePassed != AT[j]) {		//jab tak Arrival time na aaye tab tak timePassed ma time add karta rahay ga
							TimePassed++;
						}	
						timeline[i+1].setText("" + TimePassed);
					}
					else if(AT[j] <= TimePassed) {
						GC[i].setVisible(true);
						GCPid[i].setText("P" + pID[j]);
						TimePassed = TimePassed + BT[j];
						timeline[i+1].setText("" + TimePassed);
						j++;
					}
				}
